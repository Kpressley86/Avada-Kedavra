$(document).ready(function () {
  ...
  var API = {
    saveExample: function(newBeerReservation) {
      return $.ajax({
        headers: {
        ...
        data: JSON.stringify(newBeerReservation)
      });
    },
  ...
  var refreshExamples = function () {
    API.getExamples().then(function (data) {
      var $examples = data.map(function (newBeerReservation) {
        var $a = $("<a>")
          .text(example.text)
  ...
  var handleFormSubmit = function (event) {
    ...
    if (!($email.text)) {
      alert("You must fill out all feilds first!");
      return;
    }

    API.saveExample(newBeerReservation).then(function () {
      refreshExamples();
    });
